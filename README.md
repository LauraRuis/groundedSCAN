...
This will visualize the predictions in `predict.json` and visualize the execution in a gif in the specified output directory. The file `example_prediction.json` contains 1 data example with a prediction that can be used for trying out the error analysis and execute commands modes.
